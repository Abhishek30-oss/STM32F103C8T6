...
void Reset_Handler(void)
{
	...
	uint32_t size = (uint32_t)&_esram - (uint32_t)&_ssram;
	
	uint8_t *pSRC = (uint8_t *)&_etext;
	uint8_t *pDST = (uint8_t *)&_ssram;
	...
	//initilaize .bss to sramm
	uint32_t bsssize = (uint32_t)&_esram_bss - (uint32_t)&_ssram_bss;
	
	pDST = (uint8_t*)&_ssram_bss;
	...
	//call init std library only in case if print f library
	//call main()
	main();
